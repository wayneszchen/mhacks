...
class Login:
    ...
    def login_browser(self):
        ...
        # Wait for successful login by monitoring URL changes and captcha completion
        import time
        max_wait_time = 300  # 5 minutes max wait
        start_time = time.time()

        print("🔄 Waiting for LinkedIn login completion (including any captcha)...")
        while True:
            current_url = driver.current_url

            # Check if we're still on login page or captcha/challenge page
            if ("login" in current_url or
                "challenge" in current_url or
                "captcha" in current_url or
                "checkpoint" in current_url or
                "uas/authenticate" in current_url):
                print(f"🔄 Still on login/captcha page: {current_url}")
                time.sleep(3)  # Wait longer for user to complete captcha
                continue

            # Check if user has successfully logged in (reached main LinkedIn pages)
            if ("linkedin.com/feed" in current_url or
                "linkedin.com/in/" in current_url or
                ("linkedin.com" in current_url and "login" not in current_url)):
                print("✅ LinkedIn login and captcha completed successfully!")
            ...
            if time.time() - start_time > max_wait_time:
                print("⚠️ Login timeout reached, attempting to continue...")
                break

            time.sleep(2)  # Check every 2 seconds

        print("🍪 Extracting LinkedIn session cookies...")
